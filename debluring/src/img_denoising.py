...
def block_denoise(img_box):
    ...
    coder = SparseCoder(dictionary=blur_dict, transform_n_nonzero_coefs=3, transform_alpha=None, transform_algorithm='omp')
    r_code = coder.transform(r_blur_patch)
    g_code = coder.transform(g_blur_patch)
    b_code = coder.transform(b_blur_patch)
    r_clear_patch = np.dot(r_code, clear_dict)
    g_clear_patch = np.dot(g_code, clear_dict)
    b_clear_patch = np.dot(b_code, clear_dict)
...
def image_denoise(image_path):
    ...
    for index_, (contour_, box) in enumerate(text_boxes):
        ...
        clear_block = block_denoise(text_block)
